...
def CSP(data1, data2, center=True):
    ...
    #get rank
    rank1 = (s > (_np.max(s) * _np.max(data2.shape) * _np.finfo(data2.dtype).eps)).sum()
    W = W1[:,:rank1] / s[:rank1][_np.newaxis] # scale by singular values to get whitening matrix
    W2, s2 = _linalg.svd(W.T.dot(data1), full_matrices = False)[:2]
    rank2 = (s2 > (_np.max(s2) * _np.max([W.shape[1], data1.shape[1]]) * _np.finfo(data1.dtype).eps)).sum()
    both = W.dot(W2[:,:rank2])
    # scale by eigenvalue
...
def CCA_data(X,Y):
    ...
    if not type(X) == _np.ndarray: raise TypeError('X must be numpy array')
    if not type(Y) == _np.ndarray: raise TypeError('Y must be numpy array')
    if not X.ndim == 2: raise ValueError('X must be a 2d numpy array - dimensionality is wrong')
    if not Y.ndim == 2: raise ValueError('Y must be a 2d numpy array - dimensionality is wrong')
    P1, M1 = X.shape
    P2, M2 = Y.shape
    if M1 != M2: raise ValueError('number of observations (columns) in X and Y does not match.')
    if P1 <= 1: raise ValueError('number of Variables in X must be larger than 1')
    if P2 <= 1: raise ValueError('number of Variables in Y must be larger than 1')
    #######################################################################
    #remove mean
    ...
    Rankx = _np.sum(_np.abs(_np.diag(Rx)) > (_np.finfo(Rx.dtype).eps * _np.max([M1,P1])))
    Ranky = _np.sum(_np.abs(_np.diag(Ry)) > (_np.finfo(Ry.dtype).eps * _np.max([M2,P2])))
    if Rankx == 0: raise ValueError('Rank of X is 0 -> it should contain at least one non-constant column!')
    if Rankx < P1:
        warnings.warn(UserWarning('X is rank deficient!'))
        # take only relevant portions of Qx and Rx
        Qx = Qx[:,:Rankx]
        Rx = Rx[:Rankx,:Rankx]
    if Ranky == 0: raise ValueError('Rank of Y is 0 -> it should contain at least one non-constant column!')
    if Ranky < P2:
    ...
    #get SVD
    U, s, Vh = _linalg.svd(Qx.T.dot(Qy), full_matrices=False, compute_uv=True)
...
def CCAvReg(trials):
    ...
    avg = _np.hstack([avg]*n_trials) # repeat the average to have the same dimensionality as in trials
...
def bCSTP(trials1, trials2, num_iter=30, s_keep=2, t_keep=2, verbose=True):
    """
    ...
    if n_ch1 != n_ch2:
        raise ValueError('Number of channels must be equal in trials1 and trials2')
    if n_dp1 != n_dp2:
        raise ValueError('Epoch length must be equal in trials1 and trials2')
    if num_iter < 10:
        raise ValueError('At least 10 iterations must be done.')
    t_keep = _np.min([_np.hstack([[int(t_keep)] * 5, _np.arange(6, num_iter+1, 1)])[::-1], [int(n_dp1)]*num_iter],0)
    s_keep = _np.min([_np.hstack([[int(s_keep)] * 5, _np.arange(6, num_iter+1, 1)])[::-1], [int(n_ch1)]*num_iter],0)
    ...
    for i in xrange(num_iter):
        if verbose:
            print 'Interation %d of %d total iterations.' % (i+1, num_iter)
        ######## get spatial filter ########
        #filter temporally
        temp1 = _np.tensordot(V[-1][:,:t_keep[i]], trials1, axes = (0,1))
        temp2 = _np.tensordot(V[-1][:,:t_keep[i]], trials2, axes = (0,1))
        ...
        ######## get temporal filter ########
        temp1 = _np.tensordot(W[-1][:,:s_keep[i]], trials1, axes = (0,0))
        temp2 = _np.tensordot(W[-1][:,:s_keep[i]], trials2, axes = (0,0))
    ...
    return W, V[1:], s_eigvals, t_eigvals # V was initialized as identity matrix, so start with the 2nd element
